...
- Pre-commit runs Ruff `--fix`/format plus `mypy --strict custom_components/unifi_presence/`; run standalone mypy only for focused type checks.
...
- WebSocket health is not true until the first inbound frame; preserve `_stopped`, watchdog, reconnect, stale-runner, and reauth-restart semantics.
